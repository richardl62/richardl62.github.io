...
class dice {
    ...
    // Show the dice spinning for the given duration, then show the current number.
    ...
    state(input_state) {
        return this.number(input_state);
    }
}
...
class holdableDice extends dice {
    ...
    state(input_state) {
        if(input_state === undefined) {
            ...
            this.hold(input_state.held);
            super.state(input_state.dice);
        }
    }
}
...
class diceSet {
    ...
    spin() {
        this.dice_set.forEach(die => die.spin(spin));
    }
    ...
    state(input_state) {
        const n_dice = this.dice_set.length;
        
        if (input_state === undefined) {
            let state = new Array(n_dice);
            for(let i = 0; i < n_dice; ++i) {
                state[i] = this.dice_set[i].state();
            } 
        } else {
            assert(input_state.length == n_dice);
            for(let i = 0; i < n_dice; ++i) {
                this.dice_set[i].state(input_state[i]);
            } 
        }
    }
}
