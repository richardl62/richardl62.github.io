...
class gamesBoardSquare {
    ...
    status(new_status)
    {
        if(new_status === undefined)
            ...
            this.elem.toggleClass("game-board-square-disabled", this.gbs_status.is_disabled());

            // Kludge? If might be better to remove the counter (and
            // re-add it later if required) if it not going to be disabled.
            var player = this.gbs_status.player();
            this.counter.css("background-color",
                player ? gamesBoardPlayerColor(player) : "inherit");
        }
    }
